...
still pre-1.0 (`0.4.0`) and is **not** yet frozen under SemVer — it may change
in a future `0.x` release before the 1.0 freeze; pin a specific tag (e.g.
`tag = "v0.4.0"`) to insulate yourself from breaking changes.
...
# Cargo.toml
[dependencies]
obj-rs = { git = "https://github.com/uname-n/libobj", tag = "v0.4.0" }
...
`Cargo.toml`, e.g.
`obj-rs = { git = "https://github.com/uname-n/libobj", tag = "v0.4.0", features = ["serde"] }`.
